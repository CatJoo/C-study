...
void printfFishes();
void initData();
void decreaseWater(long elasedTime);
int checkFishAlive();
...
int main_pointer_project(void)
{
	...
	while (1)
	{
		...
		//사용자가 입력한 어항에 물을 준다
		//1. 어항의 물이 0이면? 물을 주지 않는다... 이미 물고기는 ㅠㅠ...
		if (cursor[num - 1] <= 0)
		{
			printf("%d 번 물고기는 이미 죽었습니다.. 물을 주지 않습니다\n", num);
		}
		//2. 어항의 물이 0이 아니면? 물을 준다! 단 100을 넘지 않는지 체크
		...
		{
			//물고기가 모두 ㅠㅠ
			printf("모든 물고기가 ..흑흑...ㅠㅠ");
			exit(0);
		}
		else
		{
			//최소 한마리 이상의 물고기는 남아있음
			printf("물고기가 아직 살아있어요!\n");
		}
		prevElapsedTime = totalElapsedTime;
		//10초 -> 15초 (5초: prevElapsedTime) -> 25초 (25 - 15 = 10초..?)
				//--> prevElapsedTime =totalElapsedTime
	}
	return 0;
}
...
void decreaseWater(long elasedTime)
{
	for (int i = 0; i < 6; i++)
	{
		arrayFish[i] -= (level * 3 * (int)elasedTime); //3은 난이도 조절을 위한 값
		if (arrayFish[i] < 0)
		{
			arrayFish[i] = 0;
		}
